    ...
    <div className="space-y-5 max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage who can access the CRM and their roles</p>
        </div>
      ...
      <div className="card p-4 flex gap-6 text-sm">
        <div>
          <span className="badge bg-blue-100 text-blue-800 mr-2">admin</span>
          Full access including user management
        </div>
        <div>
          <span className="badge bg-gray-100 text-gray-700 mr-2">user</span>
          Can view and enter data, no admin access
        </div>
      </div>
            ...
            {!loading && users.map((u) => {
              ...
              return (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`badge ${u.role === 'admin' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-700'}`}>
                      {u.role}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <button
                      ...
                      title={isSelf ? 'Cannot disable your own account' : u.is_active ? 'Click to disable' : 'Click to enable'}
                    >
      ...
      {showForm && (
        ...
        >
          <UserForm
            ...
            onSave={handleSave}
            onClose={() => { setShowForm(false); setEditing(null) }}
            isSelf={editing?.username === currentUser?.username}
          />
        </Modal>
      )}
    </div>
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      {initial && (
        <div className="flex items-center gap-2">
          ...
          {isSelf && <span className="text-xs text-gray-400">(cannot disable yourself)</span>}
        </div>
      )}
